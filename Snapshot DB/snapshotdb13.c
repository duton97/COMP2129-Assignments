...
int* uniq_algo(int* array, int* oldLength){
	...
	int newLength = i + 1;
	int tempIndex = 0;
	int* ptr = (int *) malloc((newLength + 1) * sizeof(int));//Plus 1 to add length at the beginning of the array

	ptr[0] = newLength; //Put length at the first index
	//Indicates that there are no values in the array except for the length, which is at index 0 with value of 0
	//if (newLength == 1) {
	//	return ptr;
	//}
