...
listSnapshot copy_entry_list(entry* headEntry, entry* tailEntry){
	...
	entry* temp = headEntry->next;

	while (temp != tailEntry){ //Iterate over the current existing list of entries
...
void command_snapshot(snapshot* headSP, snapshot* tailSP, entry* headEntry, entry* tailEntry){
	...
	listSnapshot tempList = copy_entry_list(headEntry, tailEntry); //Copy and make a new list
	headEntry = tempList.head; //Update head to head of the new list
	tailEntry = tempList.tail; //Update tail to tail of the new list
