...
void command_set(entry* head, entry* tail, entry* entry, int *array, int *length, char key[]) {
	...
	//Fill in the entry struct with the given inputs (key, values and length)
		...
		printf("Memory address of head: %p\n", head);
		printf("Memory address of entry: %p\n", entry);
	}
}
...
void command_drop(snapshot* headSP, snapshot* tailSP, int* id) {
	...
	snapshot* tempSP = check_same_id(headSP, tailSP, id);
	entry* temp = tempSP->entries->next; //Point to first entry

	//Iterate from first entry to tail, exclude the head
	while (temp->next != NULL) {
		printf("Memory address of headEntry in drop: %p\n", temp);
		temp->next->prev = temp->prev; //Set current's next entry to point to current's previous entry
		temp->prev->next = temp->next; //Set current's previous entry to point to current's next entry
		free(temp->values);
		free(temp);
		temp = tempSP->entries->next; //Update temp
	}
	snapshot_del(tempSP);
	printf("ok\n\n");
}
...
listSnapshot copy_entry_list(entry* head, entry* tail){
	...
	listSnapshot list;
	list.head = tempHeadEntry;
	list.tail = tempTailEntry;
	...
	headEntry = tempHeadEntry;
	tailEntry = tempTailEntry;
	return list;
}
...
void command_checkout(snapshot* headSP, snapshot* tailSP, entry* headEntry, entry* tailEntry, int* id){
	...
	//listSnapshot tempList = copy_entry_list(tempHead, tempTail);
	//headEntry = tempList.head;
	//tailEntry = tempList.tail;

	printf("ok\n\n");
}

void command_snapshot(snapshot* headSP, snapshot* tailSP, entry* headEntry, entry* tailEntry){
	printf("Memory address of headEntry: %p\n", headEntry);
	printf("Memory address of tailEntry: %p\n", tailEntry);
	snapshot* temp = create_snapshot(headSP, tailSP);
